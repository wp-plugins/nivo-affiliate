...
if (!class_exists('nap_pvw_plugin_framework')) {

class nap_pvw_plugin_framework {
	...
	function pvw_get_debug_info($type) {
		...
		$data = '';	
		$data .= '<b>================ '.__('Debug Data',NAP_PVW_PLUGIN_LINK).' - '.NAP_PVW_PLUGIN_NAME.' ====================</b><br/><br/>';
		$data .= '<b>==WordPress==</b><br/><br/>';
		$data .= '<b>'.__('Version',NAP_PVW_PLUGIN_LINK).': </b>'.$wp_version.'<br/>';
		$data .= '<b>'.__('URL',NAP_PVW_PLUGIN_LINK).': </b>'.$home_url.'<br/>';
		$data .= '<b>'.__('Install',NAP_PVW_PLUGIN_LINK).': </b>'.$wp_url.'<br/>';
		$data .= '<b>'.__('Language',NAP_PVW_PLUGIN_LINK).': </b>'.$language.'<br/>';
		$data .= '<b>'.__('Charset',NAP_PVW_PLUGIN_LINK).': </b>'.$charset.'<br/>';
		$data .= '<br/><br/>';
		$data .= '<b>=='.__('Extra Info',NAP_PVW_PLUGIN_LINK).'==</b><br/><br/>';
		$data .= '<b>'.__('PHP Version',NAP_PVW_PLUGIN_LINK).': </b>'. $php_version.'<br/>';
		$data .= '<b>'.__('Server Software',NAP_PVW_PLUGIN_LINK).': </b>'. $_SERVER['SERVER_SOFTWARE'].'<br/>';
		$data .= '<b>'.__('User Agent',NAP_PVW_PLUGIN_LINK).': </b> '.$_SERVER['HTTP_USER_AGENT'].'<br/>';
		$data .= '<b>'.__('cURL Init',NAP_PVW_PLUGIN_LINK).': </b>'. $curl_init.'<br/>';
		$data .= '<b>'.__('cURL Exec',NAP_PVW_PLUGIN_LINK).': </b>'. $curl_exec.'<br/>';
		$data .= '<br/><br/>';			
		$data .= '<b>=='.__('Theme',NAP_PVW_PLUGIN_LINK).'==</b><br/><br/>';
		$data .= '<b>'.__('Name',NAP_PVW_PLUGIN_LINK).': </b>'.$theme_name.'<br/>';
		$data .= '<b>'.__('URI',NAP_PVW_PLUGIN_LINK).': </b>'.$theme_uri.'<br/>';
		$data .= '<b>'.__('Parent',NAP_PVW_PLUGIN_LINK).': </b>'.$theme_parent.'<br/>';
		$data .= '<b>'.__('Version',NAP_PVW_PLUGIN_LINK).': </b>'.$theme_version.'<br/>';
		$data .= '<br/><br/>';			
		$data .= '<b>=='.__('Active Plugins',NAP_PVW_PLUGIN_LINK).'==</b><br/><br/>';
		$data .= $plugins_string;
		...
		$data = "
================ Debug Data - ".NAP_PVW_PLUGIN_NAME." ====================
...
$plugins_string
	...
	function pvw_send_debug_data() {
		...
		$subject = NAP_PVW_PLUGIN_NAME.' Support - Debug Data';
	...
	function pvw_head_css() {
			...
			if ($css_option == 'custom') {
				...
				// Output styles
				if ($output <> '') {
					$output = "<!-- ".NAP_PVW_PLUGIN_NAME." Custom Styling -->\n<style type=\"text/css\">\n" . $output . "</style>\n";
					echo $output;
				}
	...
	function pvw_options_setup(){
		...
		foreach($template as $option) {
			if($option['type'] != 'heading' && $option['type'] != 'button' && $option['type'] != 'note' &&  $option['type'] != 'intro' &&  $option['type'] != 'donation' &&  $option['type'] != 'debug' &&  $option['type'] != 'html'){
				$id = $option['id'];
				$std = $option['std'];
		...
		foreach ($options as $value) {
			...
			case "button":
				...
				if (isset($value['std']) && $value['std'] == 'debug') {
					...
					$buttons .= '{ '.NAP_PVW_PLUGIN_CLASS.'::'.$value['method'].'();';
			...
			case "html":
				...
				$output .= '<div class="html"><p>'. $value['message'] .'</p></div>';
	...
	function pvw_plugin_settings() {
		...
		$options = get_option('pvw_'.NAP_PVW_PLUGIN_SHORTCODE.'_template');		
	...
	?>
			<div id="pvw_settings" class="wrap">	
				...
				<div class="wrap" id="pvw_container">
					...
					<form method="post" action="options.php">
						<fieldset>
							<div id="header">
								<div>
									<h2 class="logo"><?php echo NAP_PVW_PLUGIN_NAME; ?> <span> v<?php echo NAP_PVW_PLUGIN_VERSION; ?></span></h2>
								</div>
							...
							<?php 	
								settings_fields('pvw_'.NAP_PVW_PLUGIN_SHORTCODE.'_plugin_options');
							...
							<div class="save_bar_top">
								...
								<input type="submit" class="button-primary" value="<?php _e('Save Changes', NAP_PVW_PLUGIN_LINK) ?>" />
							
								<form action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" method="post" style="display:inline" id="ofform-reset">
									<span class="submit-footer-reset">
										<input name="reset" type="submit" value="<?php _e('Reset Options', NAP_PVW_PLUGIN_LINK) ?>" class="button submit-button reset-button" onclick="return confirm('<?php _e('Click OK to reset. Any settings will be lost!', NAP_PVW_PLUGIN_LINK) ?>');" />
				...
				<div id="pvw_footer">
					...
					<div id="links">
						<b><?php echo NAP_PVW_PLUGIN_NAME; ?> v<?php echo NAP_PVW_PLUGIN_VERSION; ?></b> | <?php _e('We hope you enjoy the plugin ', NAP_PVW_PLUGIN_LINK); ?>
						<br/>
						<a href="http://www.polevaultweb.com/support/forum/<?php echo  NAP_PVW_PLUGIN_LINK; ?>-plugin/" title="<?php _e('Visit the support forum for', NAP_PVW_PLUGIN_LINK) ?> <?php echo NAP_PVW_PLUGIN_NAME; ?>"><?php _e('Support Forum', NAP_PVW_PLUGIN_LINK) ?></a> |
					...
					<div id="pvw">
						<a id="logo" href="http://www.polevaultweb.com/" title="Plugin by polevaultweb.com" target="_blank"><img src="<?php echo NAP_PVW_PLUGIN_URL; ?>admin/images/pvw_logo.png" alt="polevaultweb logo" width="190" /></a>
					</div>
